...
if echo "$RAW_JSON" | grep -q "dl_speed"; then
    ...
    # 1. PARSING: Använd JQ istället för osäker eval/awk
    ...
    # 2. ASSIGN: Dela upp värden till variabler på ett säkert sätt
    IFS="|" read -r DL UL PNG PMIN PMAX JIT SRV DST SENT MAX <<EOF
$PARSED
EOF

    # 3. FORMAT: Avrundning och beräkningar
    ...
    # 4. PACKET LOSS: Jonas logik men utan eval
    ...
    # 5. CONTENT: Skapa Discord-meddelandet
...
# 6. SEND: Skicka till Discord med en JQ-byggd payload (säkrar mot specialtecken)
